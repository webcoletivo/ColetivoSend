...
/**
 * Tela pública de download (/send/d/<token>) — o page.tsx (servidor) já
 * garantiu que o token existe.
 *
...
export function DownloadClient({ token }: { token: string }) {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    setPasswordError('')
    try {
      const unlockRes = await fetch(`/api/transfer/${token}/unlock`, {
        method: 'POST',
  ...
  // Conta o download, avisa o dono (sino + e-mail da plataforma) e devolve
  // URLs presignadas NOVAS. As URLs recebidas ao abrir a página valem 15 min;
  // se a pessoa demorou, a rota renova. Espera pouco (2,5 s) e, se a rota não
  // responder a tempo (rede, limite), usa a URL que já estava em mãos —
  // falha aqui nunca impede o download.
  const registrarDownload = async (fileId?: string): Promise<Map<string, string>> => {
    const urls = new Map<string, string>()
    try {
      const controle = new AbortController()
      const tempo = setTimeout(() => controle.abort(), 2_500)
      ...
      clearTimeout(tempo)
      if (res.ok) {
        const data = await res.json() as { downloads?: { id: string; url: string }[] }
        for (const d of data.downloads ?? []) if (d?.id && d?.url) urls.set(d.id, d.url)
      }
    } catch {
      // segue com as URLs já recebidas
    }
    return urls
  }
  ...
  const handleDownloadFile = async (file: TransferFile) => {
    setBaixando(file.id)
    try {
      const novas = await registrarDownload(file.id)
      dispararDownload(novas.get(file.id) ?? file.downloadUrl)
    } finally {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      const novas = await registrarDownload()
